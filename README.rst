...
* `tests`: These are general tests that are hard or annoying to write with only
  the minimal dependencies, so we don't have any tooling restrictions here.
  `tox -e py36-full` will run these, as well as all the other test sets
